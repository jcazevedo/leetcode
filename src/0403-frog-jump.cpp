...
class Solution {
 private:
  bool goodAux(int stone,
    ...
    if (cache.find(stone) == cache.end() || cache[stone].find(lastJump) == cache[stone].end()) {
      bool ans = false;
      if (!ans && lastJump - 1 > 0) ans = goodAux(stone + lastJump - 1, lastJump - 1, positions, lastStone, cache);
      if (!ans && lastJump > 0) ans = goodAux(stone + lastJump, lastJump, positions, lastStone, cache);
      if (!ans) ans = goodAux(stone + lastJump + 1, lastJump + 1, positions, lastStone, cache);
      cache[stone][lastJump] = ans;
    }
    return cache[stone][lastJump];
  }

  bool good(int stone, int lastJump, const unordered_set<int>& positions, int lastStone) {
    unordered_map<int, unordered_map<int, bool>> cache;
