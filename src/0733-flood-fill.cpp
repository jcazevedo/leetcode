...
private:
  ...
  void flood_fill(vector<vector<int>>& image, int sr, int sc, int H, int W,
    ...
    for (pair<int, int> new_dir : dirs) {
      ...
      if (new_sr >= 0 && new_sr < H && new_sc >= 0 && new_sc < W &&
          image[new_sr][new_sc] == image[sr][sc] && !visited[new_sr][new_sc]) {
