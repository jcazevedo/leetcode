...
class Solution {
 private:
  int of_length(int n_digits, int length) {
    int ans = 1;
    for (int i = 0; i < length; ++i) ans *= n_digits;
    return ans;
  }
  ...
  int aux(int idx, string num, vector<string>& digits) {
    ...
    for (int i = 0; i < num_digits && digits[i][0] <= num[idx]; ++i) {
      if (digits[i][0] < num[idx]) ans += of_length(num_digits, L - idx - 1);
      if (digits[i][0] == num[idx]) ans += aux(idx + 1, num, digits);
    }
    return ans;
  }
 ...
 public:
  int atMostNGivenDigitSet(vector<string>& digits, int n) {
    ...
    int ans = 0;
    for (int i = 1; i < len; ++i) ans += of_length(n_digits, i);
