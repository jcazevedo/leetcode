...
class Solution {
private:
  int getMinTime(int curr, const vector<vector<int>>& subordinates, const vector<int>& informTime) {
    if (subordinates[curr].empty())
      return 0;
    int ans = 0;
    for (int i = 0; i < (int)subordinates[curr].size(); ++i)
      ans = max(ans, getMinTime(subordinates[curr][i], subordinates, informTime));
